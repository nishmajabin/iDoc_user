...
class UserCallBloc extends Bloc<UserCallEvent, UserCallState> {
  ...
  String? _pendingCallId, _pendingChannelName, _pendingDoctorId,
      _pendingDoctorName, _pendingDoctorProfileImageUrl, _currentUserId;
  ...
  bool _getMuted(UserCallState s) =>
      s is UserCallActive ? s.isMuted : s is UserCallWaitingForPeer ? s.isMuted : false;
  ...
  void _clearPending() {
    ...
    _callStatusSub = null;
    _pendingCallId = _pendingChannelName = _pendingDoctorId =
        _pendingDoctorName = _pendingDoctorProfileImageUrl = null;
  }

  void _watchCallStatus(String callId, {required bool whileRinging}) {
    _callStatusSub?.cancel();
    _callStatusSub = _repository.watchCallStatus(callId: callId).listen((status) {
      if (isClosed) return;
      if (whileRinging && (status == 'ended' || status == null) && state is UserCallRinging) {
  ...
  Future<void> _onStartListening(StartListeningForCalls event, Emitter<UserCallState> emit) async {
    _currentUserId = event.userId;
    await _incomingCallsSub?.cancel();
    _incomingCallsSub = _repository.watchIncomingCalls(userId: event.userId).listen((snapshot) {
      if (isClosed) return;
      for (final doc in snapshot.docs) {
        final data = doc.data();
        if (data['status'] != 'ringing') continue;
        final s = state;
        if (s is UserCallIdle || (s is UserCallRinging && s.callId != doc.id)) {
  ...
  Future<void> _onStopListening(StopListeningForCalls event, Emitter<UserCallState> emit) async {
    await _incomingCallsSub?.cancel();
    _incomingCallsSub = null;
    emit(const UserCallIdle());
  }

  void _onIncomingCall(IncomingCallReceived event, Emitter<UserCallState> emit) {
    ...
    emit(UserCallRinging(
      callId: event.callId, channelName: event.channelName,
      doctorId: event.doctorId, doctorName: event.doctorName,
      doctorProfileImageUrl: event.doctorProfileImageUrl,
    ));
  }

  void _onIncomingCallCancelled(IncomingCallCancelled event, Emitter<UserCallState> emit) {
    _clearPending();
    emit(const UserCallIdle());
  }

  Future<void> _onCallAccepted(CallAccepted event, Emitter<UserCallState> emit) async {
    ...
    if (callId == null || channelName == null || _currentUserId == null) {
      emit(const UserCallIdle()); return;
    }

    emit(UserCallConnecting(callId: callId, channelName: channelName, doctorName: doctorName));

    try {
      await _repository.acceptCall(callId: callId);
      _watchCallStatus(callId, whileRinging: false);
      await _repository.initAndJoin(
        appId: appId, channelName: channelName, userId: _currentUserId!,
        onRemoteUserJoined: (uid) { if (!isClosed) add(RemoteUserJoined(uid)); },
        onRemoteUserLeft: (uid) { if (!isClosed) add(RemoteUserLeft(uid)); },
        onError: (msg) { if (!isClosed) add(CallErrorOccurred(msg)); },
      );
      _startTimer();
      emit(UserCallWaitingForPeer(callId: callId, channelName: channelName, doctorName: doctorName));
    } catch (e) {
      emit(UserCallError(e.toString()));
    }
  }

  Future<void> _onCallRejected(CallRejected event, Emitter<UserCallState> emit) async {
    if (_pendingCallId != null) {
      try { await _repository.rejectCall(callId: _pendingCallId!); } catch (_) {}
    }
    _clearPending();
    emit(const UserCallIdle());
  }

  void _onRemoteUserJoined(RemoteUserJoined event, Emitter<UserCallState> emit) {
    final callId = _getCallId(state);
    final channelName = _getChannelName(state);
    if (callId == null || channelName == null) return;
    emit(UserCallActive(
      callId: callId, channelName: channelName, doctorName: _getDoctorName(state),
      remoteUid: event.uid, isMuted: _getMuted(state), elapsedSeconds: _elapsedSeconds,
    ));
  }
  ...
  Future<void> _onCallEndRequested(CallEndRequested event, Emitter<UserCallState> emit) async {
    _stopTimer();
    await _callStatusSub?.cancel();
    _callStatusSub = null;
    final callId = _pendingCallId ?? _getCallId(state);
    if (callId != null) {
      try { await _repository.endCallDocument(callId: callId); } catch (_) {}
    }
  ...
  Future<void> _onMuteToggled(CallMuteToggled event, Emitter<UserCallState> emit) async {
    final newMuted = !_getMuted(state);
    await _repository.muteLocalAudio(mute: newMuted);
    final s = state;
    if (s is UserCallActive) emit(s.copyWith(isMuted: newMuted));
    else if (s is UserCallWaitingForPeer) emit(s.copyWith(isMuted: newMuted));
  }

  Future<void> _onCameraSwitched(CallCameraSwitched event, Emitter<UserCallState> emit) =>
  ...
  void _onTimerTicked(CallTimerTicked event, Emitter<UserCallState> emit) {
    ...
    final s = state;
    if (s is UserCallActive) emit(s.copyWith(elapsedSeconds: event.seconds));
    else if (s is UserCallWaitingForPeer) emit(s.copyWith(elapsedSeconds: event.seconds));
  }
