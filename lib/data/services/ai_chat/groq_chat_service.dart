...
/// Model: `llama-3.1-8b-instant` (production, lightweight, fast).
class GroqChatService implements ChatApiService {
  ...
  Future<MedicalChatMessage> sendImageMessage({
    ...
    // llama3-8b-8192 doesn't support vision — send as text-only with a note.
  ...
  String _mapHttpError(int statusCode, String body) {
    switch (statusCode) {
      ...
      case 401:
      case 403:
        return 'Invalid or unauthorized API key. Please verify your Groq API key.';
      case 429:
        return 'Groq rate limit reached. Switching to backup provider...';
      case 500:
      case 502:
      case 503:
